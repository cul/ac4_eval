...
module FedoraMigrate::Hooks
  ...
  # Called from FedoraMigrate::ObjectDescriber
  def before_object_description
    # additional actions as needed
  end

  # Called from FedoraMigrate::ObjectDescriber
  def after_object_description
  end
...
desc "Run the MODS migration to properties"
  task list: :id_migrators do
    ...
    unless list && File.exists?(list)
      puts "usage: rake mods:list list=LIST_PATH"
    else
      ac = ::RDF::URI("info:fedora/ac#")
      migrator = FedoraMigrate::AcademicCommons::ListDescriber.new('ac', list:list,prefixes:{ac:ac})
